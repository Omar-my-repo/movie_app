...
class _WatchListState extends State<WatchList> {
  ...
  WatchListViewModel vm =WatchListViewModel(
                                  ...
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      SizedBox(
                                        ...
                                        child: InkWell(
                                          onTap: () {
                                            Navigator.push(context, MaterialPageRoute(builder: (context) =>DetailsScreen(movieId:myProvider.watchList[index].id.toInt()),));
                                            
                                          },
                                          child: movie_image_item(
                                            id:myProvider.watchList[index].id,
                                            date:myProvider.watchList[index].date,
                                            posterPath:myProvider.watchList[index].imageUrl,
                                            title:myProvider.watchList[index].title,
                                            description: myProvider.watchList[index].description,
                                            ),
                                        ),
                                      ),
                                        Expanded(
                                            ...
                                            children: [
                                              Container(
                                                ...
                                                child: Column(
                                                  crossAxisAlignment: CrossAxisAlignment.start,
                                                  children: [
                                                    Text(myProvider.watchList[index].title,overflow:TextOverflow.ellipsis,textAlign:TextAlign.start,),
                                                    const SizedBox(height: 5,),
                                                    Text(myProvider.watchList[index].date,overflow:TextOverflow.ellipsis,textAlign:TextAlign.start),
                                                    const SizedBox(height: 10,),
                                                    Text(myProvider.watchList[index].description,overflow:TextOverflow.ellipsis,textAlign:TextAlign.start,maxLines: 4,),
                                                  ],
                                                ))
                                            ],
                                          ),
